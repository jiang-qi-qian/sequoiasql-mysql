...
static const my_bool SDB_DEFAULT_BKA_REMOVE_ORDERBY = TRUE;
...
static const my_bool SDB_DEFAULT_ORDERBY_LIMIT_OPT = TRUE;
...
my_bool sdb_orderby_limit_optimization = SDB_DEFAULT_ORDERBY_LIMIT_OPT;
...
mysql_var_update_func sdb_set_exec_mode = NULL;
mysql_var_update_func sdb_set_exec_only_in_mysql = NULL;
...
static void sdb_exec_mode_update(THD *thd, struct st_mysql_sys_var *var,
                                 void *var_ptr, const void *save) {
  if (sdb_set_exec_mode) {
    sdb_set_exec_mode(thd, var, var_ptr, save);
  }
}

static void sdb_exec_only_in_mysql_update(THD *thd,
                                          struct st_mysql_sys_var *var,
                                          void *var_ptr, const void *save) {
  if (sdb_set_exec_only_in_mysql) {
    sdb_set_exec_only_in_mysql(thd, var, var_ptr, save);
  }
}
...
static MYSQL_THDVAR_BOOL(execute_only_in_mysql, PLUGIN_VAR_OPCMDARG,
                         ...
                         NULL, sdb_exec_only_in_mysql_update, FALSE);
...
static MYSQL_THDVAR_BOOL(
    lossy_end_range, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_HIDDEN,
    "SEQUOIASQLMAINSTREAM-1479 "
    "Ignore the end range on character strings data type fields when using "
    "index to retrieve on a given range."
    "(Default: OFF)"
    /* 字符型类型字段是否在 range 索引扫描时忽略右边界索引条件。*/,
    NULL, NULL, FALSE);

// SDB_DOC_OPT = IGNORE
static MYSQL_THDVAR_BOOL(
    cls_pushdown_expand, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_HIDDEN,
    "SEQUOIASQLMAINSTREAM-1534 "
    "Expand 'direct_count', 'direct_limit' and 'direct_sort' pushdown. "
    "(Default: ON)"
    /* SequoiaDB 优化选项开关，以决定是否扩展 count、limit、sort 下压。*/,
    NULL, NULL, TRUE);

// SDB_DOC_OPT = IGNORE
static MYSQL_THDVAR_BOOL(support_cond_not,
                         PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_HIDDEN,
                         "SEQUOIASQLMAINSTREAM-1533 "
                         "Whether support push SequoiaDB $not matcher down."
                         "(Default: ON)"
                         /* 是否支持匹配符 $not 的下压 */,
                         NULL, NULL, TRUE);
...
// SDB_DOC_OPT = IGNORE
static MYSQL_THDVAR_BOOL(
    choose_all_in_mrr_info, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_HIDDEN,
    "SEQUOIASQLMAINSTREAM-1606 "
    "Whether do all choosing job in ::multi_range_read_info[_const](). "
    "(Default: ON)"
    /* 是否在 ::multi_range_read_info[_const] 函数完成所有 mrr 选择工作 */,
    NULL, NULL, TRUE);

// SDB_DOC_OPT = IGNORE
static MYSQL_THDVAR_BOOL(
    build_sort_cond_on_runtime, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_HIDDEN,
    "SEQUOIASQLMAINSTREAM-1571 "
    "Whether build sort condition on runtime. "
    "(Default: ON)"
    /* 是否在 runtime 层构建排序条件(包括order by、group by)。*/,
    NULL, NULL, TRUE);
...
// SDB_DOC_OPT = IGNORE
static MYSQL_THDVAR_BOOL(
    enable_null_reject_for_bka_join, PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_HIDDEN,
    "SEQUOIASQLMAINSTREAM-1749 "
    "Whether enable null reject for batched keys access join or not. "
    "(Default: ON)"
    /* 是否在 BKA JOIN 中启用 null_reject */,
    NULL, NULL, TRUE);

static MYSQL_THDVAR_UINT(execution_mode, PLUGIN_VAR_NOCMDOPT,
                         "Execution mode of SQL statement. (Default: 0)"
                         /* SQL 语句的执行模式 [0, 2]。*/,
                         NULL, sdb_exec_mode_update, 0, 0, 2, 1);

static MYSQL_THDVAR_BOOL(bka_remove_orderby,
                         PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_HIDDEN,
                         "SEQUOIASQLMAINSTREAM-1478 Remove index sort from BKA"
                         "(Default: ON)"
                         /*执行 bka join 携带 index 排序*/,
                         NULL, NULL, SDB_DEFAULT_BKA_REMOVE_ORDERBY);

static MYSQL_THDVAR_BOOL(secure_log, PLUGIN_VAR_OPCMDARG,
                         "Encrypt log sensitive information when printing logs"
                         "(Default: ON)"
                         /* 打印日志时是否需要对 condition 等进行编码处理 */,
                         NULL, NULL, TRUE);

static MYSQL_THDVAR_BOOL(share_log_output,
                         PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_HIDDEN,
                         "SEQUOIASQLMAINSTREAM-1974 Whether to output share logs"
                         "(Default: OFF)"
                         /*是否打印统计信息相关日志*/,
                         NULL, NULL, FALSE);
...
// SDB_DOC_OPT = IGNORE
static MYSQL_SYSVAR_BOOL(orderby_limit_optimization, sdb_orderby_limit_optimization,
                         PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_HIDDEN,
                         "SEQUOIASQLMAINSTREAM-2025 enable the ORDER BY + "
                         "LIMIT pushdown optimization . (Default: ON). "
                         /*优化 ORDER BY + LIMIT 场景的下压*/,
                         NULL, NULL, SDB_DEFAULT_ORDERBY_LIMIT_OPT);
...
struct st_mysql_sys_var *sdb_sys_vars[] = {
    ...
    MYSQL_SYSVAR(lossy_end_range),
    MYSQL_SYSVAR(cls_pushdown_expand),
    MYSQL_SYSVAR(support_cond_not),
    MYSQL_SYSVAR(choose_all_in_mrr_info),
    MYSQL_SYSVAR(build_sort_cond_on_runtime),
    MYSQL_SYSVAR(support_cond_const_bool),
    MYSQL_SYSVAR(enable_null_reject_for_bka_join),
    MYSQL_SYSVAR(execution_mode),
    MYSQL_SYSVAR(bka_remove_orderby),
    MYSQL_SYSVAR(secure_log),
    MYSQL_SYSVAR(share_log_output),
    MYSQL_SYSVAR(stats_persistence),
    MYSQL_SYSVAR(orderby_limit_optimization),
    NULL};
...
int sdb_execution_mode(THD *thd) {
  return THDVAR(thd, execution_mode);
}

void *sdb_execution_mode_ptr(THD *thd) {
  return &(THDVAR(thd, execution_mode));
}

void *sdb_execute_only_in_mysql_ptr(THD *thd) {
  return &(THDVAR(thd, execute_only_in_mysql));
}

void sdb_set_execution_mode(THD *thd, int val, bool cascade_update) {
  THDVAR(thd, execution_mode) = val;
  if (cascade_update && thd && val < SDB_SQL_EXEC_MODE_SYNC_DDL_IN_SQL_NODES) {
    THDVAR(thd, execute_only_in_mysql) = (bool)val;
  }
}
...
void sdb_set_execute_only_in_mysql(THD *thd, bool val, bool cascade_update) {
  THDVAR(thd, execute_only_in_mysql) = val;
  if (cascade_update && thd &&
      THDVAR(thd, execution_mode) < SDB_SQL_EXEC_MODE_SYNC_DDL_IN_SQL_NODES) {
    THDVAR(thd, execution_mode) = val;
  }
}
...
bool sdb_bka_using_index_sort(THD *thd) {
  return !THDVAR(thd, bka_remove_orderby);
}
...
int sdb_lossy_end_range(THD *thd) {
  return THDVAR(thd, lossy_end_range);
}

int sdb_cls_pushdown_expand(THD *thd) {
  return THDVAR(thd, cls_pushdown_expand);
}

bool sdb_support_cond_not(THD *thd) {
  return THDVAR(thd, support_cond_not);
}

bool sdb_choose_all_in_mrr_info(THD *thd) {
  return THDVAR(thd, choose_all_in_mrr_info);
}

bool sdb_build_sort_cond_on_runtime(THD *thd) {
  return THDVAR(thd, build_sort_cond_on_runtime);
}
...
bool sdb_enable_null_reject_for_bka_join(THD *thd) {
  return THDVAR(thd, enable_null_reject_for_bka_join);
}

bool sdb_use_secure_log(THD *thd) {
  return THDVAR(thd, secure_log);
}

bool sdb_share_log_output(THD *thd) {
  return THDVAR(thd, share_log_output);
}
