...
error:
  ...
  if (new_cs) {
    ...
    drop_cl(cs_name, cl_name, mapping_ctx, true);
    new_cl = false;
  }
  goto done;
}
...
int Sdb_conn::rename_cl(const char *db_name, const char *old_tbl_name,
                        const char *new_tbl_name, Mapping_context *old_ctx) {
  int rc = 0;
  const char *cs_name = db_name;
  const char *old_cl_name = old_tbl_name;
  const char *new_cl_name = new_tbl_name;
  bool need_rename_cl = true;

  if (old_ctx) {
    rc = Name_mapping::get_mapping(db_name, old_tbl_name, this, old_ctx);
    ...
    cs_name = old_ctx->get_mapping_cs();
    old_cl_name = old_ctx->get_mapping_cl();
    need_rename_cl = !old_ctx->is_specified_mapping();
  }
...
int Sdb_conn::swap_cl(const char *db_name, const char *old_table_name,
                      const char *new_table_name, Mapping_context *old_ctx,
                      Mapping_context *new_ctx) {
  int rc = 0;
  const char *cs_name = db_name;
  const char *old_cl_name = old_table_name;
  const char *new_cl_name = new_table_name;
  char backup_name[SDB_CL_NAME_MAX_SIZE + 1] = {0};
  bool mapping_swapped = false;

  DBUG_ASSERT((!old_ctx && !new_ctx) || (old_ctx && new_ctx));
  if (old_ctx && new_ctx) {
    rc = Name_mapping::get_mapping(db_name, old_table_name, this, old_ctx);
    if (0 != rc) {
      goto error;
    }
    cs_name = old_ctx->get_mapping_cs();
    ...
    rc = Name_mapping::get_mapping(db_name, new_table_name, this, new_ctx);
    if (0 != rc) {
      goto error;
    }
    new_cl_name = new_ctx->get_mapping_cl();
  }

  // swap mapping configuration if they are different
  if (old_ctx && new_ctx &&
      (old_ctx->is_part_table() != new_ctx->is_part_table() ||
       old_ctx->is_specified_mapping() != new_ctx->is_specified_mapping())) {
    rc = Name_mapping::swap(this, old_ctx, new_ctx);
    if (0 != rc) {
      goto error;
    }
  ...
  sprintf(backup_name, "#tmp-%s", old_cl_name);
  // rename old table to backup_name
  rc = retry(boost::bind(conn_rename_cl, m_connection, cs_name, old_cl_name,
                         backup_name));
  rc = (SDB_DMS_NOTEXIST == get_sdb_code(rc) ? 0 : rc);
  if (0 != rc) {
    goto error;
  }

  // rename new table to old table name
  rc = retry(boost::bind(conn_rename_cl, m_connection, cs_name, new_cl_name,
                         old_cl_name));
  rc = (SDB_DMS_NOTEXIST == get_sdb_code(rc) ? 0 : rc);
  if (0 != rc) {
    // try to rollback the first rename table operation
    if (retry(boost::bind(conn_rename_cl, m_connection, cs_name, backup_name,
                          old_cl_name))) {
      SDB_LOG_WARNING("Failed to rename '%s' to '%s'", backup_name,
                      old_cl_name);
    }
    goto error;
  }

  // rename backup_name to new table name
  rc = retry(boost::bind(conn_rename_cl, m_connection, cs_name, backup_name,
                         new_cl_name));
  rc = (SDB_DMS_NOTEXIST == get_sdb_code(rc) ? 0 : rc);
  if (0 != rc) {
    // try to rollback the upper rename table operations
    int tmp_rc = retry(boost::bind(conn_rename_cl, m_connection, cs_name,
                                   old_cl_name, new_cl_name));
    if (tmp_rc) {
      SDB_LOG_WARNING("Failed to rename '%s' to '%s'", old_cl_name,
                      new_cl_name);
    }
    if (!tmp_rc && retry(boost::bind(conn_rename_cl, m_connection, cs_name,
                                     backup_name, old_cl_name))) {
      SDB_LOG_WARNING("Failed to rename '%s' to '%s'", backup_name,
                      old_cl_name);
    }
    goto error;
...
error:
  if (mapping_swapped) {
    if (Name_mapping::swap(this, old_ctx, new_ctx)) {
      SDB_LOG_WARNING(
          "Failed to rollback swap mapping configuration from '%s' to '%s'",
...
int conn_drop_cl(sdbclient::sdb *connection, const char *cs_name,
                 const char *cl_name, const bson::BSONObj *options) {
  int rc = SDB_ERR_OK;
  sdbclient::sdbCollectionSpace cs;
  ...
  try {
    ...
    // 'options' may include 'SkipRecycleBin' option if it's not empty.
    // 'SkipRecycleBin' option will be set in 'sdb_build_recycle_bin_option' if
    // Recycle Bin function is supported
    if (options && !options->isEmpty()) {
      rc = cs.dropCollection(cl_name, *options);
    } else {
...
int Sdb_conn::drop_cl(const char *db_name, const char *table_name,
                      Mapping_context *mapping_ctx, bool skip_recycle_bin) {
  int rc = 0;
  const char *cs_name = db_name;
  const char *cl_name = table_name;
  bson::BSONObj options = SDB_EMPTY_BSON;
  if (NULL != mapping_ctx) {
    rc = Name_mapping::get_mapping(db_name, table_name, this, mapping_ctx);
    if (HA_ERR_END_OF_FILE == rc) {
      rc = 0;
  ...
  // build drop collection options
  rc = sdb_build_recycle_bin_option(*this, options, skip_recycle_bin);
  if (0 != rc) {
    goto error;
  }
  rc = retry(
      boost::bind(conn_drop_cl, m_connection, cs_name, cl_name, &options));
  if (0 != rc) {
    goto error;
  }
  if (NULL != mapping_ctx) {
...
Sdb_simple_pool_conn::~Sdb_simple_pool_conn() {
  close();
}

int Sdb_simple_pool_conn::get_connection(Sdb_conn **sdb_conn) {
  int rc = SDB_ERR_OK;
  static int MAX_TRY_TIMES = 3;
  int try_times = 0;
  m_lock.lock();
  while (try_times < MAX_TRY_TIMES) {
    try_times++;
    if (!m_idle_list.empty()) {
      *sdb_conn = m_idle_list.front();
      m_idle_list.pop_front();
      if (!(*sdb_conn)->is_connected()) {
        SDB_LOG_INFO(
            "Connection in simple connection pool is not valid, try to "
            "reconnect");
        rc = (*sdb_conn)->reconnect();
        if (0 != rc) {
          SDB_LOG_ERROR("Failed to reconnect, error: %s",
      ...
      m_working_list.push_back(*sdb_conn);
      break;
    }

    // too many connections, wait other thread release connection
    if (m_actives >= m_max_conn_num) {
      m_lock.unlock();
      sleep(1);
      m_lock.lock();
      continue;
    ...
    *sdb_conn = new (std::nothrow) Sdb_normal_conn(0, sdb_conn_str);
    if (NULL == (*sdb_conn)) {
      rc = SDB_ERR_OOM;
      goto error;
    }

    // do not set any session attributes, use default session attributes
    (*sdb_conn)->enable_raw_connect(true);
    rc = (*sdb_conn)->connect();
    if (0 != rc) {
      SDB_LOG_ERROR("Failed to connect SequoiaDB, error: %s",
                    (*sdb_conn)->get_err_msg());
      delete (*sdb_conn);
      (*sdb_conn) = NULL;
      goto error;
    }
    // push new connection into working list
    m_working_list.push_back(*sdb_conn);
    m_actives++;
    SDB_LOG_DEBUG(
        "There are %d connetions in connection pool of mapping function",
        m_actives);
    break;
  }
  if (NULL == (*sdb_conn)) {
    SDB_LOG_ERROR("Timeout while getting connection from connection pool");
    rc = SDB_ERR_WAIT_TIMEOUT;
    goto error;
  }
done:
  m_lock.unlock();
...
int Sdb_simple_pool_conn::release_connection(Sdb_conn *sdb_conn) {
  if (NULL != sdb_conn) {
    Sdb_mutex_guard guard(m_lock);
    m_working_list.remove(sdb_conn);
    m_idle_list.push_back(sdb_conn);
  }
  return SDB_ERR_OK;
}

int Sdb_simple_pool_conn::close() {
  Sdb_mutex_guard grard(m_lock);
  DBUG_ASSERT(m_working_list.empty());
  for (std::list<Sdb_conn *>::iterator it = m_idle_list.begin();
       it != m_idle_list.end(); it++) {
    delete (*it);
  }
  return SDB_ERR_OK;
}
