...
class Sdb_conn {
 public:
  ...
  int rename_cl(const char *cs_name, const char *old_cl_name,
                const char *new_cl_name, Mapping_context *old_ctx = NULL);

  int swap_cl(const char *cs_name, const char *old_cl_name,
              const char *new_cl_name, Mapping_context *mapping_ctx = NULL,
              Mapping_context *new_ctx = NULL);
  ...
  int real_connect(const char *conn_addr);
  ...
  void enable_raw_connect(bool raw_connect) { m_enable_raw_conn = raw_connect; }
...
class Sdb_simple_pool_conn {
 public:
  Sdb_simple_pool_conn(int max_conn_num = SDB_DEF_MAX_CONN_NUM)
      : m_actives(0), m_max_conn_num(max_conn_num) {}
  ~Sdb_simple_pool_conn();

  int get_connection(Sdb_conn **conn);
  int release_connection(Sdb_conn *conn);
  int close();

 private:
  Sdb_mutex m_lock;
  std::list<Sdb_conn *> m_idle_list;
  std::list<Sdb_conn *> m_working_list;
  int m_actives;
  int m_max_conn_num;
};
...
class Replaced_stat_cursor : public Stat_cursor {
 public:
  ...
  int next(bson::BSONObj &obj, bool getOwned) {
    ...
    if (0 == res) {
      // mysql_file_fgets will not update errno
      if (0 == strlen(str)) {
