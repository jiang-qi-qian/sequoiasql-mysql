...
/**
 * @Description seqDB-28626:create table
 *              like:mysql为复合分区表，sdb为主子表，主表为range分区表，子表为hash分区表，sdb源表和目标表集合属性不一致
 * @Author xiaozhenfan
 * @CreateDate 2022/11/14
 * @UpdateAuthor xiaozhenfan
 * @UpdateDate 2022/11/14
 */
public class CreateTableLike28626 extends MysqlTestBase {
    private String dbName = "db_28626";
    ...
    public void test() throws Exception {
        ...
        // 其中主表为range分区，子表为hash分区表；指定不同的集合属性
        String tbName1 = "tb_28626_1";
        String tbName2 = "tb_28626_2";
        ...
        jdbc.update( "create table " + tbName1 + " (id int) COMMENT='sequoiadb:"
                ...
                + "AutoSplit:false,Partition: 8,ReplSize:2, Compressed:true,"
        ...
        jdbc.update( "create table " + tbName2 + " (id int) COMMENT='sequoiadb:"
                + "{table_options:{EnsureShardingIndex:false,"
                + "AutoSplit:false,Partition: 8,ReplSize:2, Compressed:true,"
                + "CompressionType:\"lzw\"," + "StrictDataMode:true}}'\n"
                + "PARTITION BY RANGE (id)\n" + "SUBPARTITION BY HASH (id)\n"
                + "SUBPARTITIONS 1 (\n"
        ...
        // 在mysql端以create table like t2 like t1的方式创建复合分区表t2，预期失败
        try {
        ...
        // 检查sdb端源表目标表集合属性(预期EnsureShardingIndex不一致，其他属性一致)；
        // 主表
        CreateTableLikeUtils.compareCollectionAttribute( sdb, dbName, tbName1,
                tbName2, new String[] { "EnsureShardingIndex" } );
        // 子表
