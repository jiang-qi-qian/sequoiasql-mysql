...
/**
 * @Description seqDB-28624:create table
 *              like:mysql为range分区表，sdb为主子表，主表为range分区表，子表为普通表，sdb源表和目标表集合属性不一致
 * @Author xiaozhenfan
 * @CreateDate 2022/11/14
 * @UpdateAuthor xiaozhenfan
 * @UpdateDate 2022/11/14
 */
public class CreateTableLike28624 extends MysqlTestBase {
    private String dbName = "db_28624";
    ...
    public void test() throws Exception {
        // 通过mysql端，创建满足sdb端要求的主子表t1（源表），t2（目标表），指定不同的集合属性
        String tbName1 = "tb_28624_1";
        String tbName2 = "tb_28624_2";
        ...
        jdbc.update( "create table " + tbName1 + " (id int) COMMENT='sequoiadb:"
                ...
                + "AutoSplit:false,Partition: 8,ReplSize:1, Compressed:true,"
        ...
        jdbc.update( "create table " + tbName2 + " (id int) COMMENT='sequoiadb:"
                + "{table_options:{EnsureShardingIndex:true,"
                + "AutoSplit:false,Partition: 8,ReplSize:1, Compressed:true,"
                + "CompressionType:\"snappy\"," + "StrictDataMode:true}}'\n"
                + "PARTITION BY RANGE (id) (\n"
                + "  PARTITION p0 VALUES LESS THAN (10)\n" + ") ;" );
        // 对t1，t2表执行dml操作
        ...
        // 检查sdb端源表目标表集合属性不一致（预期为CompressionType和CompressionTypeDesc不一致，其余属性一致）；
        // 主表
        CreateTableLikeUtils.compareCollectionAttribute( sdb, dbName, tbName1,
                tbName2, new String[] { "CompressionType", "CompressionTypeDesc" } );
        // 子表
        CreateTableLikeUtils.compareCollectionAttribute( sdb, dbName,
                tbName1 + "#P#p0", tbName2 + "#P#p0",
                new String[] { "CompressionType", "CompressionTypeDesc" } );
    }
