...
/**
 * @Descreption seqDB-34099:创建不同类型的表，加载统计信息
 * @Author chenzejia
 * @CreateDate 2024.06.06
 */
public class StatsCache_34099 extends MysqlTestBase {

    private String dbName = "db_34099";
    private String tbName1 = "tb_34099_1";
    private String tbName2 = "tb_34099_2";
    private String rangeTbName = "tb_34099_3";
    private String listTbName = "tb_34099_4";
    private String hashTbName = "tb_34099_5";
    private String keyTbName = "tb_34099_6";
    private String compoundTbName = "tb_34099_7";
    ...
    public void test() throws Exception {
        ...
        // 清除mysql端缓存
        StatsCacheUtils.clearMysqlCache( jdbc, dbName, tbName1 );
        StatsCacheUtils.clearMysqlCache( jdbc, dbName, tbName2 );
        StatsCacheUtils.clearMysqlCache( jdbc, dbName, rangeTbName );
        StatsCacheUtils.clearMysqlCache( jdbc, dbName, listTbName );
        StatsCacheUtils.clearMysqlCache( jdbc, dbName, hashTbName );
        StatsCacheUtils.clearMysqlCache( jdbc, dbName, keyTbName );
        StatsCacheUtils.clearMysqlCache( jdbc, dbName, compoundTbName );

        // mysql加载统计信息，从缓存表读取统计信息
        checkReadHACL( jdbc, "select * from " + tbName1 + " where a>'str1'" );
        checkReadHACL( jdbc, "select * from " + tbName2 + " where a>'str1'" );
        checkReadHACL( jdbc, "select * from " + rangeTbName + " where c>'a'" );
        checkReadHACL( jdbc,
                "select * from " + listTbName + " where name>'0'" );
        checkReadHACL( jdbc, "select * from " + hashTbName
                + " where c2='abc'and c3='2002-02-14'" );
        checkReadHACL( jdbc,
                "select * from " + keyTbName + " where c3>'2002-02-14'" );
        checkReadHACL( jdbc, "select * from " + compoundTbName
    ...
    public void checkReadHACL( JdbcInterface jdbc, String sql )
            throws Exception {
        ArrayList< String > loadStatSqls = new ArrayList<>();
        loadStatSqls.add( sql );
